    ...
    <view class="container">
        <view class="order" v-for="shopItem in props.orderInfo.shopList" :key="shopItem.shopId" @click="toOrderDetail">
            ...
            <view class="entity_box" v-for="entityItem in shopItem.entityList" :key="entityItem.entityId">
                <view class="entity_good">
                    ...
                    <view class="entity_good_text">
                        ...
                        <view class="entity_good_text_bottom">
                            <view class="price_number">￥{{ (entityItem.paidAmount / 100)}}</view>
                            <view class="price_quantity">
                                X{{entityItem.quantity}}
                            </view>
                        </view>
                    </view>
                </view>
                ...
                <view class="order_type">
                    <view class="order_type_text" v-if="props.orderInfo.actionableList.includes('pay')">
                    ...
                    <view class="order_type_text" v-else-if="props.orderInfo.status == 131075">
                        <TnCountDown
                        separator-color="#666666"
                        size="24"
                        :time="props.orderInfo.countDown"
                        :show-hour="true"
                        :show-day="true"
                        :show-minute="false"
                        :show-second="false"
                    ...
                    <!--  -->
                    <view class="refund_detail_box" v-if="entityItem.refundedQuantity > 0" @click.stop="torefundDetail(entityItem)">
						<view class="refund_detail_text">退款详情</view>
						<image
							class="refund_detail_icon"
							:src="getAssetsUrl('/shop/left-icon.png')"
							mode="scaleToFill"
						/>
					</view>
                </view>
        ...
        <view class="order-botom" v-if="isnotDel">
            ...
            <view class="order-allbtn">
                ...
                <view class="order-allbtn-contact btn-border-grey" @click="toAftersalesList" v-if="props.orderInfo.actionableList.includes('after_sale')">
                    <text class="order-allbtn-contact-text">申请售后</text>
                </view>
                <view class="order-allbtn-contact btn-border-grey" @click="toOrderDetail" v-if="props.orderInfo.actionableList.includes('view_express')">
                    <text class="order-allbtn-contact-text">查看物流</text>
                </view>
                <view class="order-allbtn-contact btn-border-green" @click.stop="delay" v-if="props.orderInfo.actionableList.includes('delay_receive')">
                    <text class="order-allbtn-contact-text">延迟收货</text>
                </view>
                <view class="order-allbtn-contact btn-border-green" @click.stop="takeDelivery" v-if="props.orderInfo.actionableList.includes('confirm_receive')">
                    <text class="order-allbtn-contact-text">确认收货</text>
                </view>
                <view class="order-allbtn-contact btn-border-green" @click="toOrderDetail" v-if="props.orderInfo.actionableList.includes('pay')">
                    <text class="order-allbtn-contact-text">去支付</text>
                </view>
                <view class="order-allbtn-contact btn-border-kong" @tap.stop="clickComment" v-if="props.orderInfo.actionableList.includes('comment')" >
...
const emit = defineEmits(["showNotify", "cancelOrder"])
...
const toOrderDetail = () => {
    GoodsOrderDetail(props.orderInfo.id)
}

const clickComment = () => {
    gotoCommentGoods({ id: props.orderInfo.id })
}

// 申请售后
const toAftersalesList = () => {
    gotoAftersalesList()
}
        ...
        success: res => {
            if (res.confirm) {
                confirmReceipt({
                    ...
                    emit('showNotify', '收货成功')
                }).catch((err) => {
                    emit('showNotify', err.message, true)
                })
            }
        }
    })
