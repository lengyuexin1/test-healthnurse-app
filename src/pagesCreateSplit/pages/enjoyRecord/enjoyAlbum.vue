    ...
    <view class="container">
                ...
                <view class="top_nav_box">
                    ...
                    <view class="sort_box">
                        <view class="sort_type_box">
                            ...
                            <TnSwitch v-model="data.selectValue" active-color="#29C86F" size="sm"
                                      @change="changeSwitch"/>
                        </view>
                        <view class="save_text">
                            ...
                            <text class="save_btn" v-else @click="downloadCheck">确认下载({{
                                    data.saveUrlList.length
                                }})
                            </text>
                        </view>
                    </view>
            ...
            <view class="page_content">
                <view class="cloudPhoto_item" v-for="item in data.dataList" :key="item.id">
                    ...
                    >
                        ...
                        <view v-for="(imgitem,index) in item.combined" :key="index" @click="previewItem(imgitem,index)"
                              class="item_box">
                            <view class="item_checkbox" v-if="data.saveBtn">
                                <TnCheckbox size="lg" checked-shape="circle" active-color="#EA3E1A"
                                            v-model="imgitem.checked" @change="changeCheck"></TnCheckbox>
                            </view>
                            <image
                            ...
                            <view class="combined_item" :class="{ 'have_right_img' : item.combined.length < 3 }" v-else>
                                <video v-if="getplatFrom() == 'MP-WEIXIN'" :src="imgitem.value" :controls="false"
                                       :show-center-play-btn="false" object-fit='contain'></video>
        ...
        <TnPopup v-model="data.showDelPopup" :overlay-opacity="1">
            <view class="video_box">
                <video v-if="getplatFrom() == 'MP-WEIXIN'" :src="data.previewUrl" controls autoplay loop
                       :show-fullscreen-btn="false" object-fit='contain'></video>
        ...
        <yk-authpup ref="authpup" type="top" @changeAuth="downloadFun"
