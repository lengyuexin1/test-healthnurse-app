<template>
    <view class="container">
		...
		>
            <template #top>
                <template v-if="!data.needCropping">
            ...
            <template v-if="!data.needCropping">
                <view class="article_page" v-if="!data.isPreview">
                    ...
                    <view class="upFile_box" v-else>
                        ...
                        <view class="up_img_inp" @click="showPreImg(data.videoImg, 0)" >
                            <view  class="up_box">
                                <view class="up_text" @click.stop="openUp">
                                    修改封面
                                </view>
                            </view>
                            <image
            ...
            <template v-else>
                <view class="cropping_content_box">
                    <view class="cropping_img_box">
                        <image
                            class="cropping_img"
                            :src="data.imgFileList[data.corppingIndex]"
                            mode="aspectFill"
                        />
                    </view>
                    <!-- <view class="cropping_img_list">
                        <view class="upFile_list">
                            <view class="upFile_item" v-for="(item, index) in data.imgFileList" :key="index">
                                <image
                    ...
                    <view class="cropping_img_list">
                        <view class="upFile_list">
                            <view class="upFile_item" v-for="(item, index) in data.imgFileList" :key="index">
                                ...
                                />
                                <view class="fixed_number">{{ index + 1 }}</view>
                                <view class="fixed_del" @click="listDel(index)">
                                    <TnIcon name="close" color="#fff" size="24" :bold="true"/>
                                </view>
                            </view>
                            <view class="upload_box" v-if="data.imgFileList.length < 9" @click="addImg">
                                <TnIcon name="add" color="#fff" size="42" :bold="true"/>
                            </view>
                        </view>
                    </view>
                </view>
            ...
            <template #bottom>
                ...
                <template v-else>
                    <view class="cropping_next_box">
                        <view class="corpping_box" @click="tocorpping">
                            <TnIcon name="shears" color="#666666" size="42" :bold="true"/>
                            <view class="corpping_text">裁剪</view>
                        </view>
                        <view class="next_btnBox" @click="corppingNext">
                            下一步
                        </view>
                    </view>
                </template>
        ...
        <TnOverlay v-model:show="data.showFullImg" :duration="0" :opacity="1" :overlay-closeable="false" :zIndex="30080">
            <view class="full_img_box">
                <image
                    class="full_img"
                    :src="data.fullImg"
                    mode="aspectFit"
                />
                <view class="full_bottom">
                    <view class="full_btn" v-if="data.upType == 3" @click="tofirst">设为首图</view>
                    <view class="full_btn" @click="tocorpping">编辑</view>

                    <view class="full_btn" v-if="data.upType == 3" @click="todel">删除</view>
                </view>
            </view>
        ...
        <TnPopup  v-model="data.showDelbox" :duration="0" :opacity="1" :overlay-closeable="false" :zIndex="30080">
            <view class="del_box">
                <view class="del_title">确认删除图片?</view>
                <view class="del_bottom">
                    <view class="del_btn out_del" @click="data.showDelbox = false">取消</view>
                    <view class="del_btn to_del" @click="todelBtn">确认</view>
                </view>
            </view>
        ...
        <l-clipper v-if="data.showclipper" @success="onok" @cancel="oncancle" :image-url="data.imgurl" :width="500" :height="500"></l-clipper>
...
// import kpsImageCutter from "@/User/components/kspImageCutter/kspImageCutter.vue"
...
onLoad((option:any)=> {
    ...
    if (option.isChange == 1) {
        ...
        newgetDetail({
            ...
            data.keywordItem = res.baseInfo.keyword ? res.baseInfo.keyword[0] : null

            data.tagIds = res.baseInfo.tagIds ? res.baseInfo.tagIds[0] : null
            data.tagNames = res.baseInfo.tagNames ? res.baseInfo.tagNames[0] : null
            data.circleId = res.baseInfo.circleId
            data.discussionName = res.baseInfo.discussionName

            console.log('data.tagIds',data.tagIds);
            console.log('data.tagNames',data.tagNames);
...
const clickBtn = () =>{
    ...
    let upInfo = {
        baseInfo: {
            ...
            tagIds: data.tagIds ? [data.tagIds] : null,
            tagNames: data.tagNames ? [data.tagNames] : null,
...
const corppingNext = () => {
    data.needCropping = false
}

const fullScreen = (src:string, index:number) => {
    data.corppingIndex = index
...
const tocorpping = () => {
    ...
    if (data.upType == 3) {
        data.imgurl = data.imgFileList[data.corppingIndex]
    }else{
        data.imgurl = data.videoImg 
    }

    data.showclipper = true
...
const onok = (e:any) => {
    ...
    uploadFileApi(e.url,name,'merchant/license').then((res:any) => {
        // res.url

        if (data.upType == 3) {
            data.imgFileList[data.corppingIndex] = res.url
        }else{
            data.videoImg = res.url
        }
...
const showPreImg = (src:string, index:number) => {
    console.log('展开src',src);

    data.showFullImg = true
    data.fullImg = src
    data.corppingIndex = index

    // data.fullIndex = index
}

const tofirst = () => {
    const element = data.imgFileList.splice(data.corppingIndex, 1)[0];
    data.imgFileList.unshift(element);
}

const toedit = () => {
    console.log('data.corppingIndex ',data.corppingIndex);
}

const todel = () => {

    if (data.imgFileList.length == 1) {
        bcNotify.value.show('至少保留一张作为封面')
        return
    }
...
const todelBtn = () => {
    data.imgFileList.splice(data.corppingIndex, 1);
...
const addImg = () => {
    uni.chooseImage({
        ...
        success: (res:any) => {
            ...
            uploadFileApi(res.tempFilePaths[0],name,'merchant/license').then((res:any) => {
                data.imgFileList = [
                    ...data.imgFileList,
                    res.url,
                ]
