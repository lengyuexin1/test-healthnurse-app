<template>
    <view class="WaterFall_item" :class="{ 'is_left' : props.direction == 1 , 'is_right' : props.direction == 2 }" @click="todetail(item)">
        ...
        <view class="WaterFall_text_box">
            <view class="WaterFall_text_title">{{ item.name ? item.name : item.title }}</view>
            <view class="WaterFall_text_bace" v-if="false">您最需要的养生宝典</view>
            <view class="rate_box" v-if="false">
                <TnRate v-model="data.rateValue" size="sm" gutter="4" active-color="#FF983D"/>
            </view>
            <view class="WaterFall_price" v-if="false">
                <view class="price_icon">¥</view>
                <view class="price_number">356.00</view>
                <view class="price_icon">/213讲</view>
            </view>
