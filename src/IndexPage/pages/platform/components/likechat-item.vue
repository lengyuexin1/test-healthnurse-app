<template>
    <view class="WaterFall_item" @click="todetail(item,index)">
        <image
            class="WaterFall_img"
            :src="item.thumb"
            mode="aspectFill"
...
interface Events {
    (e: 'clickwaterItem', item: any, index:number, direction:number): void
}
...
const todetail = (item:any,index:number) =>{
    emit('clickwaterItem',item,index,props.direction)
}
