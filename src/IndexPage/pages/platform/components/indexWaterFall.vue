             ...
             <view id="left" v-if="data.leftList.length">
                <template v-for="(item,index) in data.leftList" :key="item.id">
                    <view  class="wf-item">
                        ...
                        <indexFollowItem v-if="isfollow" :isSmall="false" :direction="1" :item="item" @clickwaterItem="clickwaterItem"/>
                        <indexWaterFallItem v-else :isSmall="false" :direction="1" :item="item" @clickwaterItem="clickwaterItem"/>
                    </view>
                </template>
             </view>
         </view>
         ...
         <view>
             <view id="right" v-if="data.rightList.length">
                <template v-for="(item,index) in data.rightList" :key="item.id">
                    <view   class="wf-item">
                        <!-- :is_statistics="is_statistics" -->
                        <indexFollowItem v-if="isfollow" :isSmall="index == 0" :direction="2" :item="item" @clickwaterItem="clickwaterItem"/>
                        <indexWaterFallItem v-else :isSmall="index == 0" :direction="2" :item="item" @clickwaterItem="clickwaterItem"/>
                    </view>
                </template>
             </view>
         </view>
...
import indexWaterFallItem from './indexWaterFallItem.vue';
import indexFollowItem from './indexFollowItem.vue'
