<template>
    ...
    <view class="wrap">
        <view class="tabs_wrap tn-flex-row">
            <view :class="['item tn-flex-column', data.currentTabIndex == index ? 'active' : '' ]" v-for="(item, index) in data.tabsData" :key="index" @tap="tabsChange(index, item.name)">
                <view>{{ item.name }}</view>
                <view class="num">
                    <text class="amount">{{ item.num }}</text>{{ item.unit }}
                </view>
            </view>
        </view>

        <view class="echarts_wrap">
            <view class="title">{{ data.currentTabName }}</view>

            <view class="data_Echarts">
                <lechart ref="chartRef"></lechart>
            </view>

            <view class="tn-flex-row" v-if="data.currentTabIndex == 0">
                <view :class="['type tn-flex-center-center', data.typeIndex == index ? 'active' : '' ]" v-for="(item, index) in data.typeData" :key="index" @tap="clickType(index)">
                    <TnIcon name="success-circle-fill" :color="data.typeIndex == index ? '#F56547' : '#CCCCCC'" :size="35" />
                    <text>{{ item.name }}</text>
                </view>
            </view>
        </view>
...
// #endif
import { liveStat, liveTrendData } from '@/api/live-api'
...
const formatData = () => {
    ...
    if (data.currentTabIndex == 0) {
        if (data.typeIndex == 0) {
            filterData = data.liveTrendData['viewStat']
        }
        else if (data.typeIndex == 1) {
            filterData = data.liveTrendData['fansViewStat']
        }
    }
    else if (data.currentTabIndex == 1) {
        filterData = data.liveTrendData['newFansStat']
    }
    else if (data.currentTabIndex == 2) {
        filterData = data.liveTrendData['durationStat']
    }
...
const onChange = (value) => {
    ...
    getLiveStat(data.startTime, data.nowTime)
    getLiveTrendData()
}

const tabsChange = (index, name) => {
    data.currentTabIndex = index
    data.currentTabName = name
    getLiveTrendData()
}
...
const getLiveStat = (startTime, endTime) => {
    liveStat({
        channelId: props.liveInfo.id,
        startTime,
        endTime
    }).then((res) => {
        data.tabsData[0].num = res?.totalUsers ?? 0
        data.tabsData[1].num = res?.totalFans ?? 0
        data.tabsData[2].num = res?.totalLiveDuration ?? 0
    }).catch((err) => {
...
.tabs_wrap {
    ...
    .active {
        ...
        box-shadow: 0rpx 4rpx 8rpx 2rpx rgba(31,8,3,0.08);
