    ...
    <view class="container">
        ...
        >
            <template #top>
                ...
                <view class="navbar_box" :style="{ paddingTop: data.titleTop + 'px', paddingRight: data.titleRight + 'px' }">
                    <view class="top_box" :style="{ height: data.sBarHeight + 'px' }" >
                        ...
                        <view class="page_share" @click="sharePage">
                            <image
                                class="share_img"
            ...
            <view class="partition_box">
                <image
                    class="partition_img"
                    :src="data.partitionObj.thumb"
                    mode="scaleToFill"
                />
                <view class="partition_text">
                    <view class="partition_title">{{ data.partitionObj.name }}</view>
                    <view class="partition_desc">{{ data.partitionObj.desc }}</view>
                    <view class="partition_number">{{ data.partitionObj.cntPost }}人讨论</view>
                </view>
            </view>

            <view class="content_list" v-if="data.dataList.length != 0">
                <view class="content_List_item" v-for="item in data.dataList" :key="item.id" @click="toDetail(item)">
        ...
        <view class="fixed_upBtn" @click="toUpdiscuss">
            + 发起讨论
        </view>
...
onLoad((option:any) => {
    ...
    //         console.log('data.partitionObj',data.partitionObj);
    //     })
    // })
...
.fixed_upBtn{
    ...
    box-sizing: border-box;
    border-radius: 36rpx;
    background: linear-gradient( 90deg, #EA3E1A 0%, #EA3E1A 100%);
    color: #FFFFFF;
