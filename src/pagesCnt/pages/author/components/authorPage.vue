<template>
    <view class="container">
        ...
        >
            <template #top>
                <PageTopbg :zIndex="-1"
                           :bgstyle="'background:linear-gradient(180deg, #FDD9CD 20%, #F2F3F5 100%)'"></PageTopbg>
                ...
                <view class="flex_author_box">
                        ...
                        <view class="author_right_box">
                            <view class="author_name_box">
                                ...
                                <view class="btn_box" v-if="!data.pageisuser">
                                    <view class="btn chat_btn" :class="{'is_fanschat' : data.pageFanstype }"
                                          @tap="clickPrivateLetter">私信
                                    </view>
                                    <view class="btn follow_btn" :class="{ 'is_fans' : data.pageFanstype }"
                                          @click="toFans">
                    ...
                    <view class="author_desc" v-if="data.homeObj.accountDescription">{{
                            data.homeObj.accountDescription
                        }}
                    </view>
                    <view class="data_box">
            ...
            <template
                  v-if="(data.tagId == 3 && (data.dataList.length == 0 && data.salonList.length == 0)) || (data.tagId != 3 && data.dataList.length == 0 )">
...
import {
    ...
    follow,
    userLetterCreator,
    getJoinsalonList, escontentlist
} from '@/api/create-api'
...
const queryList = (pageNumber: number, pageSize: number) => {
    ...
    escontentlist({
        query: {
            typeIds: data.tagList[data.navIndex].type,
            // happyType: data.tagId ? data.tagId : null,
            // status: 3,
            accountId: props.accountId
        },
