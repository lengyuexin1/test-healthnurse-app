    ...
    <view class="cl-updata">
        <view :style="[listRowStyle]" class="file-list">
            ...
            <view v-for="(item, index) in previewList" :key="index" :style="[rowStyle]"
                ...
                <image
                      v-if="imageType == 'image' || fileUrlType(item) === 'image'"
                      :src="item.path"
                      :style="[imgStyle]"
                      class="_image"
                      mode="aspectFill">
            ...
            <view v-if="add && FileList.length < max" :style="[rowStyle]" class="file-list-row"
        ...
        <view v-if="tempVideoUrl" class="mask">
            <image :src="closeImg" class="_root" mode="widthFix" @tap="closeTempVideoUrl"></image>
...
export default {
    ...
    methods: {
         ...
         * */
        previewImage(item) {
            if (this.fileUrlType(item) === 'video') {
                return false
            }
        ...
        isBase64(str) {
            if (str === '' || typeof str !== 'string') {
                return console.error('文件路径错误, base64', str)
            }
