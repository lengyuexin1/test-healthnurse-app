...
const emit = defineEmits(["selectCoup"])
...
const getCoupList = () => {
	...
	console.log('props.coupsList',props.coupsList);
    ...
    props.list.map((x) => {
        if (x.canUse == 1) {
			...
			console.log('isNowCoupon',isNowCoupon);
...
const InitCpup = (index:number) => {
	data.coupIndex = index
    getCoupList()

	data.coupId.push(data.coupArr[0])
	
    emit('selectCoup',data.coupIndex, data.coupArr[0], true)
}
...
const changeGroup = (e:any) => {
	...
	console.log('切换选中优惠券',item);
	

    emit('selectCoup',data.coupIndex,item[0])
}

defineExpose({ openCpup, InitCpup, closeCpup })
</script>
