        ...
        <view class="order_info_top" @click="clickGotoServeDetail(serviceInfo.info.itemId)">
            ...
            <view class="order_info_shop_box" v-if="!isinstitution">
                ...
                <view class="order_info_shop">
                    服务由
                    <text class="shop_name">{{ serviceInfo.shopinfo.shopName }}</text>
                    提供
                </view>
            </view>
        </view>
            ...
            <view class="service_info_item" v-if="serviceInfo.addressInfo">
                ...
                <view class="info_text_box">
                    <view class="info_text">
                        <text>{{ serviceInfo.addressInfo.area }}{{ serviceInfo.addressInfo.address }}</text>
                    </view>
                    <view class="info_text">
                        <text>{{ serviceInfo.addressInfo.name }} {{ serviceInfo.addressInfo.mobile }}</text>
                    </view>
                </view>
            </view>
            ...
            <div class="record_box" v-if="[131079,196611].includes(serviceInfo.status)">
                ...
                <div class="record_list">
                    <div class="record_title_box">
                        ...
                        <div class="use_box"
                             v-if="serviceInfo.info.refundableQuantity && serviceInfo.info.refundableQuantity > 0">
                            <div class="use_time">{{ showutcTime(serviceInfo.info.consumeCardList) }}</div>
                            <div class="use_state">过期</div>
                        </div>
                    </div>
                    <div v-for="(item,index) in serviceInfo.info.consumeCardList" :key="index"
                         @click="tominutes(index)">
                        <div class="record_item" v-for="(recordItem) in item.cardCodes" :key="recordItem">
                            <div class="record_title"
                                 :class="{ 'isuse': item.status == 196611 || item.status == 196613 }">{{ recordItem }}
                            </div>
                            <div class="use_box">
