...
import { getCollageRecord } from '@/api/goods-api'
...
const getDetail = (orderId:string) => {
    getserviceOrderDetail({
        ...
        // 获取拼团倒计时
        if (res.collageRecordId && res.status == '131089') {
            getCollageRecord({ recordId: res.collageRecordId }).then((pac: any) => {
                data.osObj.utcResidual = pac.utcResidual
            })
        }
    })
}
