	...
	<TnPopup v-model="show" open-direction="bottom" @close="close" @open="open" round="40rpx" closeable>
        <view class="content">
				...
				<view class="gods row">
					...
					<view class="godsrig column">
						...
						<view class="godpic row">
							<text class="godnow" v-if="[3, 4].includes(mode)">￥{{ moneyFilter(joinInfo.collagePrice) }}</text>
							<text class="godnow" v-else>￥{{ moneyFilter(data.comInfo.price) }}</text>
						</view>
					</view>
				</view>
            ...
            <view class="peopbtn" v-if="!isCar">
				...
				<template v-else>
					<TnButton bg-color="#FFD07E" text-color="#fff" width="48%" :customStyle="singleBuyBtnStyle" shape="round" @click="balance">
						￥{{ Number(data.comInfo.price) / 100 }}单独购买
					</TnButton>
					<TnButton bg-color="#FC3848" text-color="#fff" width="48%" :customStyle="groupBuyBtnStyle" shape="round" @click="balance(1)">
						￥({{ joinInfo.collagePrice ? (Number(joinInfo.collagePrice) / 100) : 0 }}) {{ joinInfo.cntCollage || 0 }}人拼团购买
					</TnButton>
				</template>
            </view>
...
import { goodsSpecifications, createGoodcart, getGroupBuyInfo } from '@/api/goods-api'
...
const singleBuyBtnStyle = {
    height: '90rpx',
    fontSize: '30rpx',
	marginRight: '2%'
}
const groupBuyBtnStyle = {
    height: '90rpx',
    fontSize: '30rpx',
	marginLeft: '2%'
}
...
// 打开弹窗
const godOpen = (type: boolean, info: any, pattern?: any) => {
    isCar.value = type
	joinInfo.value = info
	mode.value = pattern
    show.value = true
...
const balance = (type?: number) => {
    ...
    const uniqueId = TempStorage.savewx(type == 1 ? {
		collageId: joinInfo.value.collageId,
		cntCollage: joinInfo.value.cntCollage,
		recordId: joinInfo.value.id,
		joinTheteamType: joinInfo.value.joinTheteamType,
		...params
	} : params)
	...
	let payJSON = JSON.stringify(payInfo)
