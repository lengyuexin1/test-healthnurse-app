        ...
        <view class="balance_info_box">
            <view class="balance_info_left">
                <image
                    ...
                    :src="agencyObj.mainPics && agencyObj.mainPics[0]"
                    mode="aspectFill"
                />
            </view>
            <view class="balance_info_right">
                <view class="balance_info_right_box">
                    <view class="balance_info_right_title">{{ agencyObj.name }}</view>
                    <view class="balance_info_right_bace" v-if="agencyObj.isRestrictTime == 0">周一至周日 过期退</view>
                    <view class="balance_info_right_bace" v-else>{{ agencyObj.restrictTimeDesc }}不可用 过期退</view>
                    <view class="balance_info_right_price_box">
                        <view class="balance_info_right_price">
                            ￥{{ agencyObj.price / 100 }}
                        </view>
                        <view>
                            <TnNumberBox v-model="data.vModelValue" bg-color="#f5f5f5" :max="agencyObj.isRestrictBuyQuantity ? agencyObj.restrictBuyQuantity :data.maxStock"
                            :min="1" :step="1" buttonSize="56rpx" inputWidth="66rpx" @change="changevalue">
                            </TnNumberBox>
                        </view>
                ...
                <view class="balance_info_right_shop">
                    <!-- <text>服务由</text>
                    <text class="shop_name">{{ props.agencyObj.shopname }}</text>
                    <text>提供</text> -->
                </view>
            </view>
        </view>
