                    ...
                    <view class="contbox">
                        <view class="log_head">
                            <view class="log_hed_li row i-center j-between">
                                <view><text class="log_hed_tit">{{data.expressObj.com}}</text>
                                <text class="log_hed_tex">{{data.expressObj.nu}}</text></view>
                                <view class="log_hed_copy row i-center j-center" @click="fuzhi(data.expressObj.nu)">复制</view>
                            </view>
                        </view>
                        <view class="log_content">
                            <view class="zong">
                                <view class="user_box" style="width: 100%; margin: 40rpx auto;">
                                    <view class="user_for" v-for="(list,id) in data.expressInfo" :key="id">
                                        <view class="line_box">
                                            <!-- 圆球 -->
...
import { realtimeLookExpressDetail } from '@/api/goods-api'
...
onLoad((option:any)=>{
    data.com = option.com
    data.num = option.num
    getLogisticsList(data.com, data.num)
})


const bcNotify = ref()
const getLogisticsList = (com:string, num:string) => {
    realtimeLookExpressDetail({
        com,
        num
    }).then((res:any) => {
        data.expressObj = res
        data.expressInfo = res.data
        data.emptyExpre = !res.data.length
    }).catch((err:any) => {
