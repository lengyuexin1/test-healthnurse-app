...
import shareView from '@/Service/components/shareView/shareView.vue'
    ...
    PlatformManage.getToken().then((token: any) => {
        createTeam({
            ...
            gotoChatPage({
                to: res.tid,
                scene: 'customer',
                originPage: 'Service/pages/Servicestore/Servicestore'
            })
        }).catch((err) => {
...
const sharePage = () => {
    ...
    uni.share({
        ...
        miniProgram: {
            ...
            path: `/Service/pages/Servicestore/ServicestoreDetail?shopId=${data.detailObj.shopId}`, //点击链接进入的页面
...
const sharePoster = async () => {
    ...
    // 二维码链接图片
    let qrimg =  await getQrcode(`/Service/pages/Servicestore/ServicestoreDetail?shopId=${data.detailObj.shopId}`).then((img:any)=>{
        return img
    })
