...
const sharePage = () => {
    ...
    uni.share({
        ...
        miniProgram: {
            ...
            path: `/Service/pages/Servicestore/healthproductDetails?itemId=${data.agencyObj.id}`, //点击链接进入的页面
...
const sharePoster = async () => {
    ...
    // 二维码链接图片
    let qrimg =  await getQrcode(`/Service/pages/Servicestore/healthproductDetails?itemId=${data.agencyObj.id}`).then((img:any)=>{
        return img
    })
