...
const tolike = (item:any, index:number) => {
    ...
    livegetLike({
        ...
        item.isLike++;
        enjoyCardRef.value[data.storageIndex].getlikeList()
        console.log('enjoyCardRef.value[data.storageIndex]',enjoyCardRef.value[data.storageIndex]);
...
const upComment = () => {
    ...
    data.isReply ? pushrecover({
        ...
        enjoyCardRef.value[data.storageIndex].getcommentList()
    }).catch((err:any)=>{
        bcNotify.value.error(err)
    }) : pushreview({
        articleId: data.storageObj.id,
