    ...
    >
        ...
        <view class="salon_list">
            <view class="salon_item" v-for="item in data.dataList" :key="item.id">
                ...
                <view class="card_box" :class="{ 'not_bottom' : item.audit == null }">
                    <salonCard :salonItem="item"></salonCard>
                </view>
                <view class="manage_btn_box" v-if="item.audit">
                    <view class="utcCreated_time">
                        {{ formatTime(item.utcCreated) }}
                    </view>
                    <view class="btn_list"> 
                        <view class="manage_btn" v-if="[1,2,3,5].includes(item.audit.audit)" @click="toUpsalonPostsPage(item)">
                            修改
                        </view>
                        <view class="manage_btn" v-if="[5].includes(item.audit.audit)" @click="tosalonPostsDetailPage(item)">
                            详情
                        </view>
                        <view class="manage_btn" v-if="[2].includes(item.audit.audit)" @click="delcircle(item)">
                            撤销
                        </view>
                        <view class="manage_btn" v-if="[5].includes(item.audit.audit)" @click="toapplydismissed(item)">
                            申请解散
                        </view>
                        <view class="manage_btn" v-if="[6].includes(item.audit.audit)" @click="todelapplycancel(item)">
                            取消申请
                        </view>
                        <view class="manage_btn" v-if="[3].includes(item.audit.audit)" @click="showresult(item)">
                            审核原因
                        </view>
                        <view class="manage_btn" v-if="[1,3].includes(item.audit.audit)" @click="todel(item)">
                            删除
                        </view>
                    </view>
                </view>
...
import { getmyCreateSalon, delcirclecancel, delsalon, applydismissed, delapplycancel } from '@/api/create-api'
import { gotoUpsalonPostsPage, gotoSalonDetail} from '@/routes/create-routes'
...
const toUpsalonPostsPage = (item:any) => {
...
const toapplydismissed = (item:any) =>{
...
const todelapplycancel = (item:any) => {
    Debounce(()=>{
        delapplycancel({
            id:item.audit.id
        }).then(()=>{
