<template>
    ...
    >
        <template #top>
            <TnTabs v-model="data.currentTabIndex" :scroll="false" :bottom-shadow="false" font-size="30rpx" active-font-size="31rpx" color="#666666" bar-color="#EA3E1A" active-color="#EA3E1A" @change="tabsChange">
        ...
        <view class="container" :class="[data.isEdit ? 'pb90' : '']">
            <SelectAllCancel btnName="删除" :list="data.watchHistory" :type="data.type" :listType="data.listType" :isEdit="data.isEdit" @clickItem="clickItem" @clickBtn="cancelCollect"></SelectAllCancel>
        </view>
    </z-paging>
...
import { viewList, deleteView, healthWatchHistory, browerHistoryList, delHealthWatchHistory, delBrowerHistory } from '@/api/user-api'
import { liveWatchHistory, delLiveWatchHistory } from '@/api/live-api'
import { gotoserviceDetail, gotoShopDetail, gotoServiceStore } from "@/routes/service-routes"
import { gotocourseVideo, gotoLiveShow, gotowxLive, gotosalonPostsDetailPage, gotovideoPreview, gotoarticledetails } from "@/routes/create-routes"
    ...
    if ([0, 1, 3].includes(data.currentTabIndex)) {
        ...
        if (data.sectionActive == 0) {
            ...
            browerHistoryList({
                ...
                let newArr = pack.filter((item:any) => {
                    return item.item
                })
