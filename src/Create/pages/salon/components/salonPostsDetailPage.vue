<template>
    ...
    >
        <template #top>
            <view id="topView">
                ...
                <view class="posts_author">
                    <view class="posts_author_left" @click="toauthor">
                        <image
                            class="author_img"
                            :src="data.salonDetail.accountThumb"
                            mode="scaleToFill"
                        />
                        <view class="author_text">
                            <view class="author_name">
                                {{ data.salonDetail.accountName }}
                            </view>
                            <view class="up_time">{{ timeformat(data.salonDetail.utcExamined)}}发布</view>
                        </view>
                    </view>
                    <view class="posts_author_right" @click="followFun" v-if="!data.salonDetail.isUser">
                        <view class="btn is_fans_btn" v-if="data.salonDetail.isFans">已关注</view>
        ...
        <view class="posts_content">
            <view class="swiper_box">
                <swiper class="swiper" :autoplay="true" :circular="true">
                    <swiper-item class="swiper_item" v-for="(item,index) in data.salonDetail.images" :key="index">
                        <image
                            @click="preImage(index,data.salonDetail.images)"
            ...
            <view class="salon_posts_title">
                {{ data.salonDetail.title }}
            </view>
            <view class="text_content">
                <!-- {{ data.salonDetail.desc }} -->
        ...
        <!-- :class="{ 'is_fixed' : data.isfixed }" :style="data.isfixed ? 'top:'+data.getpageTopH+'px' : ''" -->
        <view class="comment_box" id="richBox">
                ...
                <view class="comment_item" v-for="item in data.dataList" :key="item.id">
                    <view class="comment_left" >
                        ...
                        <view class="comment_text">
                            <view @touchstart="startTime(item,1)" @touchend="endTime">
                            ...
                            <view class="comment_item not_bottom" v-for="sonitem in item.commentRelyLists" :key="sonitem.id">
                                <view class="comment_left" @touchstart="startTime(sonitem,0,true,item)" @touchend="endTime">
                                    <image
                                        class="comment_img"
            ...
            <view class="tile_list" v-if="data.navId == 3">
                <view class="tile_item" v-for="item in data.followList" :key="item.id">
                    <image
                        class="tile_img"
                        :src="item.thumb"
                        mode="scaleToFill"
                    ...
                    <view class="flexinp_box">
                        ...
                        <view class="icon_list" v-if="!data.iskeyboard">
                            <view class="icon_item" @click="tolike">
                                <image
                                    class="icon_img"
                                    :src="data.salonDetail.isLike == 1 ? getAssetsUrl('/channel/like_highlight.svg') : getAssetsUrl('/channel/like.svg')"
                                    mode="scaleToFill"
                                />
                            </view>
                            ...
                            <view class="icon_item" @click="tocollect">
                                <image
                                    class="icon_img"
                                    :src="data.salonDetail.isFavorite == 1 ? getAssetsUrl('/channel/collect_highlight.svg') : getAssetsUrl('/channel/collect.svg')"
                                    mode="scaleToFill"
                                />
                            </view>
...
import { articleaddView } from '@/api/create-api'

import { gotoIndex } from "@/routes/public-routes"

import {
    getsaloncontentDetails,
    getsalonlikeList,
    getsaloncommentList,
    getsaloncommentRelyList,
    postsalonreview,
    deletesalonComment,
    postsalonrecover,
    unsalonCommentLike,
...
import { gotoauthor, gotoReportPage } from '@/routes/create-routes'
...
const queryList = (pageNumber : number, pageSize : number) => {
    ...
    if (data.navId == 1) {
        ...
        getlikeList(props.id)
    }else if (data.navId == 3){
        favoriteList(props.id)
    }
} 
...
const getdetail = (contentId:string) => {
    getsaloncontentDetails({
        ...
        data.showText = data.salonDetail.desc.replace(/\n/g, '<br>')
        ...
        bcNotify.value.error('该帖子已经被删除了')
        setTimeout(() => {
            pageController.back()
        }, 1500)
    })
...
const tocollect = () => {
    ...
    if (data.salonDetail.isFavorite == 1) {
        unsalonfavorite({
...
// 关注
const followFun = () => {
    if (data.salonDetail.isFans) {
        unsalonfollow({
            accountId:data.salonDetail.accountId
        }).then(()=>{
            getdetail(props.id)
        })
    }else{
...
const startTime = (item:any,commentType:number, sontouch: boolean = false, Topitem:any = {}) => {
    timer.value = setTimeout(()=>{

        data.sontouch = sontouch

        console.log('长按事件触发',data.sontouch);
        data.showTouch = true
        if (item.isUser == 1) {
...
const touchReply = () => {
    if (data.sontouch) {
        console.log('顶部data.storageTopItem,选中data.storageItem',data.storageTopItem,data.storageItem);
        showComment(data.storageTopItem)
        sonQuotereply(data.storageItem)
    }else{
        console.log('选中data.storageItem',data.storageItem);
...
const toauthor = () => {
    gotoauthor({
        isuser:data.salonDetail.isUser,
        accountId:data.salonDetail.accountId,
        isFans:data.salonDetail.isFans
    })
}
...
const sharePage = () => {
    ...
    uni.share({
        ...
        title: data.salonDetail.title,
        miniProgram: {
            id: 'gh_fd20b530cb94', //微信小程序原始id
            path: `/Create/pages/salon/salonPostsDetail?id=${data.salonDetail.id}`, //点击链接进入的页面
...
const sharePoster = async () => {
    ...
    const coverUrl = await drawBGIMG(data.salonDetail.cover)
    // 二维码链接图片
    let qrimg =  await getQrcode(`/Create/pages/salon/salonPostsDetail?id=${data.salonDetail.id}`).then((img:any)=>{
        return img
    })
    const qrimgUrl = await drawBGIMG(qrimg)

    const accountThumb = await drawBGIMG(data.salonDetail.accountThumb)
    ...
    let title = data.salonDetail.title//标题
    let accountName = data.salonDetail.accountName//作者，店铺名
...
const toReportPage = () => {
    ...
    let query = data.isTopcomment ? {
		...
		toUserId: data.storageItem.userId,
        objectName: `${data.storageItem.userName}: ${data.storageItem.comment}`,
        objectId: data.storageItem.replyId
	}
