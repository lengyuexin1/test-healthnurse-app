...
const KEY_TOKEN = 'IM_TOKEN_TOKEN'
//缓存
const cacheManage = CacheBuilder.uni.getInstance<IIMToken>(USER_MOD)
...
//用户授权
const loginTokenProvider = new DataProvider<IIMToken>()
//初始化
...
class IMProvider {
     ...
     */
    async setToken(loginToken: IIMToken) {
        return this.onReady().then(() => {
            //存储本地
    ...
    async clearStoreAuthorize() {
        ...
        requireLoginProvider.setData(true)
        //重置: 清空授权
        loginTokenProvider.setData(undefined as any)
        //重置状态: 清除登录成功的状态
        loginSuccessProvider.setData(undefined as any)
        return Promise.resolve('')
    }
